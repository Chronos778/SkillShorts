...
import { getUserByClerkId, getUserById, syncUserFromClerk } from '@/services/users';
...
const PublicProfile = () => {
    ...
    const followMutation = useMutation({
        mutationFn: async () => {
            if (!dbCurrentUser || !currentUser) {
                throw new Error("Please sign in to follow");
            }
            // Re-sync to ensure user ID is valid in DB
            const freshUser = await syncUserFromClerk(
                currentUser.id,
                currentUser.emailAddresses?.[0]?.emailAddress || '',
                currentUser.fullName || currentUser.firstName || 'User',
                currentUser.imageUrl
            );
            const userId = freshUser?.id || dbCurrentUser.id;
            console.log('[PublicProfile] Follow using userId:', userId, 'targetId:', id);

            let success: boolean;
            if (isFollowed) {
                success = await unfollowUser(userId, id!);
            } else {
                success = await followUser(userId, id!);
            }
        ...
        onSuccess: (nowFollowing) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['db-user-current'] });
            toast({ title: nowFollowing ? "Followed!" : "Unfollowed" });
        },
        onError: (err) => {
